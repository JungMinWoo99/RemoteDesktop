...
class RealTimeFrameHandlerThread : public AVFrameHandlerThread
{
public:
	RealTimeFrameHandlerThread(FrameDecoder& decoder, AVFrameProcessor& proc_obj) :AVFrameHandlerThread(decoder, proc_obj) {}
protected:
	void HandlerFunc() {
		SharedAVFrame recv_frame;
		while (is_processing)
		{
			while (decoder.GetRecentFrame(recv_frame))
			{
...
public:
	...
	void FrameProcess(SharedAVFrame frame) override
	{
		...
		std::this_thread::sleep_for(std::chrono::microseconds(1000000 / DEFALUT_FRAME_RATE/2));
...
int main() {	
	...
	RealTimeFrameHandlerThread frm_thr(decoding_obj, prc_obj);
	...
	AVPacket* avpkt = packet.get()->getPointer();
	do {
		av_read_frame(formatContext, avpkt);
	} while (!(avpkt->flags & AV_PKT_FLAG_KEY));
	dec_thr.InputPacket(packet);
	cout << "recv key frame done" << endl;

	
	while (true) {
		packet = MakeSharedAVStruct<AVPacket*>();
		avpkt = packet.get()->getPointer();
