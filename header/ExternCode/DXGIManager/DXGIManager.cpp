...
HRESULT DXGIOutputDuplication::AcquireNextFrame(IDXGISurface1** pDXGISurface, DXGIPointerInfo*& pDXGIPointer)
{
	...
	HRESULT hr = m_DXGIOutputDuplication->AcquireNextFrame(CAPTURE_PERIOD_MS, &fi, &spDXGIResource);
	if (FAILED(hr))
	{
	...
	hr = spDXGIResource->QueryInterface(__uuidof(ID3D11Texture2D), (void**)&spTextureResource);
	...
	hr = spD3D11Texture2D->QueryInterface(__uuidof(IDXGISurface1), (void**)&spDXGISurface);
...
HRESULT DXGIManager::Init()
{
		...
		AdapterIter++)
	{
			...
			OutputIter++)
		{
			...
			HRESULT hr = (*OutputIter)->QueryInterface(__uuidof(IDXGIOutput1), (void**)&spDXGIOutput1);
			...
			hr = spD3D11Device->QueryInterface(__uuidof(IDXGIDevice1), (void**)&spDXGIDevice);
